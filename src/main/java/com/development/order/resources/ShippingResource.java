...
private ShippingService service;
	...
	@PostMapping
	public ResponseEntity<ShippingDTO> insert (@RequestBody ShippingRequestDTO dto){
		Shipping sh = service.insert(dto);
		
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				  .buildAndExpand(sh.getId()).toUri();
		  return ResponseEntity.created(uri).body(ShippingDTO.fromDTO(sh));
	}
